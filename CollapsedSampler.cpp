...
void CollapsedSampler::init(long m, long samplesTotal, long samplesOut, long Nunmap,const TagAlignments *alignments, const distributionParameters &betaPar, const distributionParameters &dirPar, long &seed){//{{{

   Sampler::init(m,samplesTotal,samplesOut,Nunmap,alignments,betaPar,dirPar,seed);

   par1.resize(Nmap);
   par2.resize(Nmap);
   par3.resize(Nmap);
   double const1a, const1b, const2;
   const1a = beta->beta + Nunmap;
   const1b = m * dir->alpha + Nmap - 1;
   const2 = beta->alpha + Nmap - 1;
   for(long i=0;i<Nmap;i++){
      par1[i] = log(const1a + i) + log(const1b - i);
      par2[i] = log(const2 - i);
      par3[i] = log(dir->alpha + i);
   }
}//}}}*/
void CollapsedSampler::sampleZ(){//{{{
   ...
   double lProbNorm,r,sum;
   int_least32_t readsAlignmentsN;

   // double const1 = m * dir->alpha + Nmap - 1;
   // randomize order: ???
   for(i=0;i<Nmap;i++){  // XXX Nmap-1 ?
      ...
      for(j=0, k=alignments->getReadsI(i); j<readsAlignmentsN; j++, k++){
         ...
         if(alignments->getTrId(k) == 0){
            lphi[j] = alignments->getProb(k) + par1[ C[0] ];
               //log(beta->beta + Nunmap + C[0]) + 
               //log(const1 - C[0]); // this comes from division in "false part"
         }else{
            lphi[j] = alignments->getProb(k) + par2[ C[0] ] + par3[ C[alignments->getTrId(k)] ];
